...
output_dir  = '//lustre/hawcz01/scratch/userspace/rbabu/astroimage/AstroImageDetection/results'
...
def run_pipeline(max_files=None):
    ...
    if max_files is not None:
        all_files = all_files[50:max_files]
    ...
    for run_num, fname in enumerate(all_files):
        ...
        fig, ax = make_plots(
            ...
            labels=['4afgl', 'pualsar', '4haawc', '1lhaaaso'],
        )
        ...
        for radius in SMEAR_RADII:
            ...
            dog_final     = np.where(dog_low_noise > 3 * sigma_resid,
                                     dog_low_noise, 0)
            print(np.min(dog_final), np.max(dog_final))
            # DoG diagnostic plot
            try:
                fig, ax = make_plots(
            ...
            if len(ps_blobs) == 0:
                print("Rerunning with low noise threshold to check for any blobs...")
                dog_final     = np.where(dog_low_noise > 2 * sigma_resid,
                                     dog_low_noise, 0)
                print(np.min(dog_final), np.max(dog_final))
                # DoG diagnostic plot
                try:
                ...
                plt.savefig(
                    os.path.join(file_out_dir,
                                f'03_dog_r{smear_radius:.2f}_lownoisethresh.png'),
                    dpi=120, bbox_inches='tight',
                )
                plt.close(fig)
                ...
                print(f"    Raw blobs found with low noise threshold: {len(ps_blobs)}")
